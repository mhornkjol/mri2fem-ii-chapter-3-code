...
def convert_mesh(infile: Path, outfile: Path):
    ...
    cell_mesh = meshio.Mesh(
        ...
        cell_data={"subdomains": [
            msh.cell_data_dict["medit:ref"]["tetra"]]},
    )
    ...
    facet_mesh = meshio.Mesh(
        ...
        cell_data={"patches": [
            msh.cell_data_dict["medit:ref"]["triangle"]]},
    )
    facet_file = outfile.with_stem(outfile.stem + "_facets").with_suffix(".xdmf").absolute().as_posix()

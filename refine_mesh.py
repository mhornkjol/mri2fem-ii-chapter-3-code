...
parser.add_argument("-i", "--infile", dest="infile", type=Path, help="Name of input file", required=True)
parser.add_argument("-c", "--no-cell-tags", dest="tf", action="store_true", default=False, help="If True do not transfer cell markers to refined grid",
                    required=False)
parser.add_argument("-f" "--facet-infile", dest="f_infile", type=Path, help="Tagged facets that should be transferred onto refined mesh", required=False)
parser.add_argument("-r", "--refine_tag", dest="tag_list", type=int, nargs="+", help="Cells that should be refined, if empty, uniform refinement", required=False)


if __name__ == "__main__":

    args = parser.parse_args()
    refine_mesh(args.infile, args.tag_list, args.f_infile, not args.tf)
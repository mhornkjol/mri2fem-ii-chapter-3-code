...
def refine_mesh(
    file: Path,
    refinement_tags: list[int] | None = None,
    facet_file: Path | None = None,
    transfer_cell_tags: bool = False,
    grid_name: str = "Grid",
    ...
    with dolfinx.io.XDMFFile(MPI.COMM_WORLD, file, "r") as xdmf:
        mesh = xdmf.read_mesh(name=grid_name, ghost_mode=dolfinx.mesh.GhostMode.none)
        if refinement_tags is not None or transfer_cell_tags:
            if tag_name is None:
                tag_name = grid_name
            cell_tags = xdmf.read_meshtags(mesh, tag_name)
    ...
    if refinement_tags is None:
        ...
        edges = dolfinx.mesh.compute_incident_entities(mesh.topology, cells, tdim, 1)
    ...
    try:
        xdmf_out = dolfinx.io.XDMFFile(MPI.COMM_WORLD, file.with_stem(file.stem + "_refined"), "w")
    ...
    if facet_file is not None:
        ...
        refined_facettag.name = grid_name
        with dolfinx.io.XDMFFile(
            MPI.COMM_WORLD, facet_file.with_stem(facet_file.stem + "_refined"), "w"
        ) as xdmf_f:
...
parser.add_argument(
    "-i", "--infile", dest="infile", type=Path, help="Name of input file", required=True
)
parser.add_argument(
    "-c",
    "--no-cell-tags",
    ...
    action="store_true",
    default=False,
    help="If True do not transfer cell markers to refined grid",
    required=False,
)
parser.add_argument(
    "-f",
    ...
    dest="f_infile",
    type=Path,
    help="Tagged facets that should be transferred onto refined mesh",
    required=False,
)
parser.add_argument(
    "-r",
    ...
    type=int,
    nargs="+",
    help="Cells that should be refined, if empty, uniform refinement",
    required=False,
)
parser.add_argument(
    "-g",
...
parser.add_argument(
    ...
    dest="tag_name",
    type=str,
    help="Name of cell tag in file, if different from grid name",
    default=None,
    required=False,
)
...
if __name__ == "__main__":
    ...
    refine_mesh(
        args.infile, args.tag_list, args.f_infile, not args.tf, args.grid_name, args.tag_name
    )
